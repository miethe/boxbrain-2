...
All implementation, per-phase reviewer gates, and the **post-workflow adversarial verify gate**
(Phase 4 — a whole-diff claims-vs-code pass) passed inside the workflow. `autopilot.post_verify`
is `passed` (clean), or `skipped_budget` (verify was skipped because the run ran low on tokens).

> **If `autopilot.post_verify === 'skipped_budget'`**: the diff is NOT verified. Before merging, run
> the adversarial claims-vs-code pass yourself — spawn 1–2 `senior-code-reviewer` agents over
> `git diff "${BRANCH}"..HEAD` vs the plan's claims, focused on data-integrity/state-mutation bugs
> and refresh/reflection gaps — and only merge if clean. Otherwise treat as `post_verify_failed` below.
...
4. **Report to user**: tier, effort points, wave count, files changed, validation results, commit SHA,
   and the post-verify outcome (`autopilot.post_verify`). If `verify_findings` is non-empty (advisory
   medium/low items on an otherwise-clean pass), surface them so the user sees what the skeptics noted.

---

#### `status: needs_opus, reason: post_verify_failed`

The nested engine reported complete with all per-phase validators green, but the **Phase 4 verify
gate** (adversarial claims-vs-code pass over the finished diff) found confirmed critical/high defects,
or could not return a verdict (`autopilot.post_verify` is `failed` or `inconclusive`). This is the
recurring "green ≠ correct" miss two AARs caught — a data-corruption bug and a refresh/reflection gap
that survived green per-phase gates. **Do NOT merge.**

Opus actions:
- Read `verify_findings` from the report — each has `severity`, `summary`, `claim`, `code_location`,
  `mismatch`. Confirm each against the diff (`git diff "${BRANCH}"..HEAD`); do not take them on faith,
  but do not dismiss a critical without tracing the code.
- For confirmed findings: fix directly on the worktree branch (well-scoped) or re-scope to exclude the
  broken AC. For `inconclusive`: run the claims-vs-code pass yourself before deciding.
- **Re-verify after fixing** (re-run the skeptic pass or trace the claims yourself) — then merge only
  once clean, following the `status: complete` merge steps.
- If a finding is a false positive, note why in the merge commit and proceed.
...
- **Harness note:** background `Workflow` agents **ignore `EnterWorktree`** — worktree isolation comes
  from the pre-flight `git worktree add` branch (Action 3) plus git commands the agents run in the
  current tree, never from the `EnterWorktree` tool. The Phase 4 verify skeptics read the diff via
  `git`, not by switching worktrees; do the same for any manual verify pass.
...
- [ ] `autopilot.post_verify` is `passed` — or, if `skipped_budget`, a manual claims-vs-code pass ran clean before merge
