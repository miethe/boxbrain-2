...
### Living handoff doc (compaction recovery)

Progress YAML survives a *crash*; it does not, on its own, make a *context
compaction* cheap to resume from. On long multi-phase builds, keep a single living
`handoff.md` alongside the progress file:

- **One-line current state at the very top**, rewritten at every checkpoint
  (e.g. `STATE: P5 in progress — Map edges done, Legend counts WIP; branch feature/cc-v3 @ a1b2c3`).
- A short running log of decisions/lessons below it (not a transcript — the
  irreversible facts a fresh context would otherwise have to re-derive).
- **Phase-end commits** as the durable anchors the handoff line points at.

In the Command Center v3 run, a single dense `handoff.md` + phase-end commits let the
session resume mid-P5 after compaction with **zero rediscovery cost**. Make this the
default for any build expected to outlive one context window.
