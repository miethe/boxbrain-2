...
def extract_python(
    ...
    # Collect all symbols across roots using temp files to avoid overwrite clobber.
    # Each invocation of extract_symbols_python.py writes a complete JSON file;
    # running it N times with the same --output would keep only the last root's
    # symbols.  Instead, write each root to a temp file and merge after the loop.
    all_symbols: List[Dict] = []

    with tempfile.TemporaryDirectory() as tmp_dir:
        for i, extract_dir in enumerate(existing_dirs):
            tmp_output = Path(tmp_dir) / f"symbols_root_{i}.json"
            cmd = [
                sys.executable,
            ...
            if result.returncode != 0:
                ...
                if "warning" in result.stderr.lower():
                    total_warnings += 1
                # Load and accumulate symbols from this root's temp file.
                # The extractor stores paths relative to its root (e.g. "routers/foo.py"
                # when extracting from "skillmeat/api/"). Prepend the root directory's
                # path (relative to project root) so consumers see project-rooted paths
                # (e.g. "skillmeat/api/routers/foo.py").
                if tmp_output.exists():
                    try:
                        root_data = json.loads(tmp_output.read_text(encoding="utf-8"))
                        # Compute root prefix relative to project root for path qualifying
                        try:
                            root_prefix = str(extract_dir.relative_to(config._project_root))
                        except ValueError:
                            root_prefix = str(extract_dir)
                        qualified: List[Dict] = []
                        for sym in root_data.get("symbols", []):
                            sym = dict(sym)
                            raw_path = sym.get("path", "")
                            # Avoid double-prefixing if the extractor already produced a full path
                            if raw_path and not raw_path.startswith(root_prefix):
                                sym["path"] = f"{root_prefix}/{raw_path}"
                            qualified.append(sym)
                        all_symbols.extend(qualified)
                        if verbose:
                            print(f"    Collected {len(qualified)} symbols from {extract_dir}")
                    except json.JSONDecodeError as e:
                        total_warnings += 1
                        if verbose:
                            print(f"    Warning: could not parse temp output for {extract_dir}: {e}")
    ...
    # Deduplicate by (name, path, line) to handle any overlapping roots
    seen: set = set()
    deduped_symbols: List[Dict] = []
    for sym in all_symbols:
        key = (sym.get("name", ""), sym.get("path", ""), sym.get("line", 0))
        if key not in seen:
            seen.add(key)
            deduped_symbols.append(sym)

    if verbose:
        print(f"    Merged {len(all_symbols)} symbols ({len(deduped_symbols)} unique) from {len(existing_dirs)} roots")

    # Write merged output
    merged_data = {"symbols": deduped_symbols}
    output_file.parent.mkdir(parents=True, exist_ok=True)
    output_file.write_text(json.dumps(merged_data, indent=2), encoding="utf-8")
...
def check_layer_split_freshness(config, verbose: bool = False) -> PipelineResult:
    """
    Check that each enabled domain's layer-split files are non-empty.

    This is a post-extraction sanity check (non-fatal, exit code 1 not 2).
    Empty layer files indicate the LAYER_PATH_MAPPING does not match the
    actual source paths in the symbol output.
    ...
    Returns:
        PipelineResult with warnings if any layer file is empty; success otherwise.
    """
    warnings = 0
    empty_files: List[str] = []

    try:
        api_layers = config.get_api_layers() if hasattr(config, "get_api_layers") else {}
        for layer in (config.get_enabled_api_layers() if hasattr(config, "get_enabled_api_layers") else []):
            try:
                ...
                try:
                    ...
                    continue
                # Count symbols in file (supports both flat and module formats)
                if "symbols" in data:
                    count = len(data["symbols"])
                elif "modules" in data:
                    count = sum(len(m.get("symbols", [])) for m in data["modules"])
                else:
                    count = 0
                if count == 0:
                    empty_files.append(str(layer_file.name))
                    warnings += 1
            except Exception:
                pass
    except Exception:
        # Config does not expose layer information — check layer split files directly
        symbols_dir = config.get_symbols_dir() if hasattr(config, "get_symbols_dir") else Path("ai")
        for layer_file in sorted(symbols_dir.glob("symbols-api-*.json")):
            if layer_file.name in {"symbols-api.backup.json"}:
                continue
            try:
                data = json.loads(layer_file.read_text(encoding="utf-8"))
            ...
            if "symbols" in data:
                ...
                count = 0
            if count == 0:
                empty_files.append(layer_file.name)
                warnings += 1

    if empty_files:
        msg = f"Layer split files with zero symbols: {', '.join(empty_files)}"
        print(f"    Warning: {msg}")
        return PipelineResult(success=True, message=msg, warnings=warnings)

    if verbose:
        print("    All layer split files contain symbols")
...
def main() -> int:
    ...
    # Step 6: Freshness check — warn (non-fatal) on empty layer-split files
    if not args.skip_split and args.domain in ["all", "api"]:
        result = run_step(
            "check_layer_split_freshness",
            lambda: check_layer_split_freshness(config, args.verbose),
            args.verbose
        )
        results.append(("check_layer_split_freshness", result))
        total_warnings += result.warnings
        # Freshness check is non-fatal: errors never accumulate from it
