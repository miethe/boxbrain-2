...
> **Cross-project note** (Phase 3): If the sessions belong to a project other than the currently active one, use the MCP tool `ccdash_session_detail` (with `project_id`) or the repo-CLI / standalone-CLI `session get --project <project_id>` instead of `session show`. `session show` (standalone CLI) uses the active target's project context and does not accept an explicit `--project` flag. The steps below use the standalone CLI `show`/`family`/`drilldown` commands which remain correct for single-project investigations.
